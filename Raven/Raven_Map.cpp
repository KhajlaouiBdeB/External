...
namespace Raven
{
    ...
    bool Raven_Map::LoadMap(const std::string& filename)
    {
        ...
        if (!in)
        {
            TraceLog(LOG_ERROR, "Bad Map Filename");
            return false;
        }

        Clear();

        GameEntity::ResetNextValidID();
        ...
        // resize the game window to accommodate the map
        const int ExtraHeightRqdToDisplayInfo = 50;
        SetWindowSize(m_iSizeX, m_iSizeY + ExtraHeightRqdToDisplayInfo);
    ...
    void Raven_Map::PartitionNavGraph()
    {
        ...
        m_pSpacePartition = new CellSpacePartition<NavGraph::NodeType*>(m_iSizeX,
                                                                        m_iSizeY,
                                                                        RavenConfig.GetInt("NumCellsX"),
                                                                        RavenConfig.GetInt("NumCellsY"),
    ...
    {
        // render the navgraph
        if (RavenUserOpt.m_bShowGraph)
        {
            GraphHelper_DrawUsingGDI<NavGraph>(*m_pNavGraph, GraphicsContext::grey, RavenUserOpt.m_bShowNodeIndices);
        }
