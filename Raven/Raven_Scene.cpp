...
namespace Raven
{
        ...
        : m_pSelectedBot(NULL), m_bPaused(false), m_bRemoveABot(false), m_pMap(NULL), m_pPathManager(NULL), m_pGraveMarkers(NULL)
    {
        ...
        LoadMap(RavenConfig.GetString("StartMap"));
    }
    ...
    //----------------------------- AttemptToAddBot -------------------------------
    //-----------------------------------------------------------------------------
    bool Raven_Scene::AttemptToAddBot(Raven_Bot* pBot)
    {
        // make sure there are some spawn points available
        if (m_pMap->GetSpawnPoints().size() <= 0)
        {
            TraceLog(LOG_ERROR, "Map has no spawn points!");
            return false;
        }
    ...
    //-----------------------------------------------------------------------------
    void Raven_Scene::AddBots(unsigned int NumBotsToAdd)
    {
        while (NumBotsToAdd--)
        {
    ...
    //  their memory
    //-----------------------------------------------------------------------------
    void Raven_Scene::NotifyAllBotsOfRemoval(Raven_Bot* pRemovedBot) const
    {
        std::list<Raven_Bot*>::const_iterator curBot = m_Bots.begin();
        for (curBot; curBot != m_Bots.end(); ++curBot)
        {
    ...
    //--------------------------- AddBolt -----------------------------------------
    //-----------------------------------------------------------------------------
    void Raven_Scene::AddBolt(Raven_Bot* shooter, Vector2D target)
    {
    ...
    //------------------------------ AddRocket --------------------------------
    void Raven_Scene::AddRocket(Raven_Bot* shooter, Vector2D target)
    {
    ...
    //------------------------- AddRailGunSlug -----------------------------------
    void Raven_Scene::AddRailGunSlug(Raven_Bot* shooter, Vector2D target)
    {
    ...
    //------------------------- AddShotGunPellet -----------------------------------
    void Raven_Scene::AddShotGunPellet(Raven_Bot* shooter, Vector2D target)
    {
    ...
    //-----------------------------------------------------------------------------
    Raven_Bot* Raven_Scene::GetBotAtPosition(Vector2D CursorPos) const
    {
        std::list<Raven_Bot*>::const_iterator curBot = m_Bots.begin();
    ...
    //-----------------------------------------------------------------------------
    bool Raven_Scene::LoadMap(const std::string& filename)
    {
        ...
        // in with the new
        m_pGraveMarkers = new GraveMarkers(RavenConfig.GetDouble("GraveLifetime"));
        m_pPathManager = new PathManager<Raven_PathPlanner>(RavenConfig.GetInt("MaxSearchCyclesPerUpdateStep"));
        m_pMap = new Raven_Map(*this);
        ...
        {
            AddBots(RavenConfig.GetInt("NumBots"));
    ...
    //-----------------------------------------------------------------------------
    void Raven_Scene::ExorciseAnyPossessedBot()
    {
        if (m_pSelectedBot)
    ...
    //-----------------------------------------------------------------------------
    void Raven_Scene::ClickRightMouseButton(Vector2D p)
    {
        Raven_Bot* pBot = GetBotAtPosition(p);
    ...
    //-----------------------------------------------------------------------------
    void Raven_Scene::ClickLeftMouseButton(Vector2D p)
    {
        if (m_pSelectedBot && m_pSelectedBot->isPossessed())
        {
            m_pSelectedBot->FireWeapon(p);
        }
    }
    ...
    //-----------------------------------------------------------------------------
    void Raven_Scene::ChangeWeaponOfPossessedBot(unsigned int weapon) const
    {
    ...
    //------------------------------------------------------------------------------
    bool Raven_Scene::isLOSOkay(Vector2D A, Vector2D B) const
    {
    ...
    //  each point.
    //-----------------------------------------------------------------------------
    bool Raven_Scene::isPathObstructed(Vector2D A,
                                       Vector2D B,
    ...
    Raven_Scene::GetAllBotsInFOV(const Raven_Bot* pBot) const
    {
    ...
    bool Raven_Scene::isSecondVisibleToFirst(const Raven_Bot* pFirst,
    ...
    //-----------------------------------------------------------------------------
    Vector2D
    Raven_Scene::GetPosOfClosestSwitch(Vector2D botPos, unsigned int doorID) const
    {
        ...
        {
            GameEntity* trig = EntityMgr->GetEntityFromID(*it);
    ...
    void Raven_Scene::Render() const
    {
        ...
        // gfx.TextAtPos(300, WindowHeight - 70, "Num Current Searches: " + ttos(m_pPathManager->GetNumActiveSearches()));
        ...
        if (m_pSelectedBot)
        {
            if (m_pSelectedBot->isPossessed())
            {
                gfx.BluePen();
                gfx.HollowBrush();
                gfx.Circle(m_pSelectedBot->Pos(), m_pSelectedBot->BRadius() + 1);
            }
            else
            {
                gfx.RedPen();
            ...
            if (IsKeyDown('Q') && m_pSelectedBot->isPossessed())
            {
                ...
                gfx.TextAtPos(Vector2D(GetMousePosition().x, GetMousePosition().y), "Queuing");
            }
        }
    }
} // namespace Raven
