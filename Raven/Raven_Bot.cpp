...
namespace Raven
{
...

    {
        ...
        m_pWeaponSelectionRegulator = new Regulator(RavenConfig.GetDouble("Bot_WeaponSelectionFrequency"));
        m_pTargetSelectionRegulator = new Regulator(RavenConfig.GetDouble("Bot_TargetingUpdateFreq"));
        m_pTriggerTestRegulator = new Regulator(RavenConfig.GetDouble("Bot_TriggerUpdateFreq"));
        m_pVisionUpdateRegulator = new Regulator(RavenConfig.GetDouble("Bot_VisionUpdateFreq"));

        // create the targeting system
        m_pTargSys = new Raven_TargetingSystem(this);

        m_pWeaponSys = new Raven_WeaponSystem(this,
                                              RavenConfig.GetDouble("Bot_ReactionTime"),
                                              RavenConfig.GetDouble("Bot_AimAccuracy"),
                                              RavenConfig.GetDouble("Bot_AimPersistance"));

        m_pSensoryMem = new Raven_SensoryMemory(this, RavenConfig.GetDouble("Bot_MemorySpan"));
    }
    ...
    //------------------------------- SetBrain -------------------------------------
    //-----------------------------------------------------------------------------
    void Raven_Bot::SetBrain(BehaviourTree::BehaviourTree* brain)
    {
        delete m_pBrain;
        m_pBrain = brain;
    }
    ...
    {
        SetAlive();
        // TODO: implement a reset of the behaviour tree when the bot is spawned
        // m_pBrain->reset();
    ...
    {
        return Vec2DDistance(Pos(), pos) / (MaxSpeed() * Game::FPS);
    }
    ...
    void Raven_Bot::Render()
    {
        ...
        gfx.ClosedShape(m_vecBotVBTrans);

        // draw the head
        gfx.BrownBrush();
        gfx.Circle(Pos(), 6.0 * Scale().x);
        ...
        if (m_bHit)
        {
            gfx.ThickRedPen();
            gfx.HollowBrush();
            gfx.Circle(m_vPosition, BRadius() + 1);
        ...
        if (RavenUserOpt.m_bShowBotIDs)
        {
            gfx.TextAtPos(Pos().x - 10, Pos().y - 20, ttos(ID()));
        }

        if (RavenUserOpt.m_bShowBotHealth)
        {
            gfx.TextAtPos(Pos().x - 40, Pos().y - 5, "H:" + ttos(Health()));
        }
