...
namespace Raven
{
    ...
    class Raven_Bot : public MovingEntity
    {
      ...
      private:
        ...
        Raven_Scene* m_pWorld;

        // drives the bot's steering targets each update (chase a target or
        // explore) - replaces the old Goal_Think arbitration hierarchy.
        // Raw pointer (temporary, pending the framework's shared_ptr migration).
        BehaviourTree::BehaviourTree* m_pBrain;
      ...
      public:
        ...
        // calculates and applies the steering force for this time-step. Called
        // internally by Update(), but also exposed so an external controller
        //(e.g. a behaviour tree) can drive the bot's movement directly without
        // going through Update()'s Goal_Think processing.
        void UpdateMovement();
